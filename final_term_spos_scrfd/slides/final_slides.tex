...
\begin{frame}{Phạm vi của báo cáo này}
  ...
  \begin{block}{Hai hướng mở rộng}
    \textbf{SPOS-NAS} cho bước \textbf{Computation Redistribution}; \quad
...
\begin{frame}{SCRFD nên được nhìn như pipeline 3 bước}
  ...
  \begin{tikzpicture}[
    ...
    \node[box] (cr) {\textbf{Bước 1: Computation Redistribution}\\Tìm cấu hình detector tốt trong cùng vùng FLOPs};
    \node[box, right=of cr] (sr) {\textbf{Bước 2: Sample Redistribution}\\Điều chỉnh phân bố scale khi huấn luyện};
...
\begin{frame}{Computation Redistribution được làm theo hai pha}
  \begin{itemize}
    \item \textbf{Pha 1: search backbone.} Ở bước này paper cố định phần còn lại tương đối ổn định để trả lời câu hỏi riêng cho backbone: trong khung 4 stage \(C_2,C_3,C_4,C_5\), độ sâu và độ rộng nên dồn vào đâu.
    \item Các candidate backbone được sinh ra từ một template có sẵn, đo lại FLOPs, rồi chỉ những cấu hình còn nằm trong cùng vùng chi phí mới được đem đi train và so sánh.
    \item \textbf{Pha 2: search toàn mạng.} Từ backbone template thắng ở pha 1, paper mới mở rộng sang detector đầy đủ: backbone vẫn được dao động trong phạm vi hợp lý, đồng thời neck và head cũng được tinh chỉnh.
    \item Ý nghĩa của cách tách này là tránh để neck/head che mờ tác động thật sự của backbone ngay từ đầu.
  \end{itemize}
\end{frame}
...
\begin{frame}{Critical thinking về các assumption của CR}
  \begin{itemize}
...
\begin{frame}{Critical thinking về các assumption của SR}
  \begin{itemize}

...
def calc_flops_breakdown(c2_w, c2_d, c3_w, c3_d, c4_w, c4_d, c5_w, c5_d):
    ...
    # Neck Lateral Convs (considered part of the stage's compute budget)
    for i in range(4):
    ...
    # Common fixed costs (Stem, Downsamples, Head)
    H, W = 640, 640
...
df.to_csv('nas_leaderboard-dynamic-exact.csv', index=False)
...
print(f"Mean GFLOPs of Top 100: {mean_gflops_winners:.3f}")
print(f"Num comparable nets in +/- {tol}G: {len(comparable)}")
print(f"Num losers selected: {len(bottom_comparable)}")
print(f"Mean GFLOPs of selected losers: {bottom_comparable['Math_GFLOPs'].mean():.3f}")

def print_stage_stats(stage_name, col):
    top_v = top_100[col].mean()
    bot_v = bottom_comparable[col].mean()
    delta = top_v - bot_v
    print(f"Stage {stage_name} (GFLOPs): Top 100 = {top_v:.3f} vs Bot Comparable = {bot_v:.3f} | Delta = {delta:.3f}")

print("\n--- NEW FUNNEL ANALYSIS (BY GFLOPs) ---")
print_stage_stats('C2', 'C2_GFLOPs')
print_stage_stats('C3', 'C3_GFLOPs')
print_stage_stats('C4', 'C4_GFLOPs')
print_stage_stats('C5', 'C5_GFLOPs')

# Plotting GFLOPs Breakdown
stages = ['C2\n(160×160)', 'C3\n(80×80)', 'C4\n(40×40)', 'C5\n(20×20)']
...
plt.bar(x - width/2, top_means, width, yerr=top_stds, label='Top-100 (Winners)', 
        capsize=5, color='#2ecc71', alpha=0.9, edgecolor='black')
plt.bar(x + width/2, bot_means, width, yerr=bot_stds, label=f'Comparable Losers\n(~{mean_gflops_winners:.2f} GFLOPs)', 
        capsize=5, color='#e74c3c', alpha=0.9, edgecolor='black')

plt.ylabel('Stage Computation Cost (GFLOPs)', fontsize=12)
plt.title('Funnel Architecture Validation: Computation Cost\nWinners vs Equivalent-GFLOP Losers', fontsize=14)
plt.xticks(x, stages, fontsize=11)
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.legend(fontsize=11)
plt.tight_layout()
plt.savefig('fig3_funnel_analysis.png', dpi=300)
print("Saved re-analyzed plot to fig3_funnel_analysis.png")

...
\begin{frame}{Computation Redistribution được làm theo hai pha}
  \begin{itemize}
    ...
    \item Ý nghĩa của cách tách này là tránh để neck/head che mờ tác động thật sự của backbone ngay từ đầu, đồng thời giữ search space đủ lớn nhưng vẫn kiểm soát được chi phí.
  \end{itemize}
\end{frame}
...
\begin{frame}{Flow triển khai SPOS-NAS proxy}
  \begin{enumerate}
    ...
    \item Từ bảng xếp hạng đó, phân tích cấu hình của Top-10, Top-100 và nhóm thua để rút ra quy luật CR.
  \end{enumerate}
\end{frame}

\begin{frame}{Kết quả chính của SPOS-NAS}
  \begin{itemize}
    \item Nhóm tốt không phân bổ compute đồng đều; winner có xu hướng \textbf{đầu tư mạnh vào \(C_2\)} và tránh làm \(C_3\) phình quá mức.
    \item \(C_2\) giữ chi tiết không gian cho tiny faces, còn \(C_5\) bổ sung ngữ nghĩa đủ mạnh cho head.
    \item \(C_3\) là vùng dễ thành bẫy chi phí: feature map vẫn còn lớn nên FLOPs đắt, nhưng gain tăng không tương xứng nếu mở quá rộng.
    \item Khi đổi proxy target từ center-focus sang edge-focus, phân bố compute lại dịch đi, cho thấy search phản ứng với \textbf{hình học của task} chứ không chỉ học thuộc một template cố định.
  \end{itemize}
\end{frame}
...
\begin{frame}{Kết quả ASR+JSAR trên WIDER FACE}
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    Thiết lập & Easy & Medium & Hard & mAP \\
    \midrule
    Default baseline & 93.62 & 91.81 & 72.49 & 85.97 \\
    Default ASR+JSAR & 90.39 & 89.25 & 76.90 & 85.51 \\
    Paper-SR12 baseline & 93.82 & 91.90 & 73.71 & 86.48 \\
...
\begin{frame}{ASR+JSAR đang sửa đúng cái gì}
  \begin{itemize}
    ...
    \item Trên nhánh paper-SR12, ASR không đẩy phân phối về tiny một cách cực đoan hơn nữa; nó chủ yếu làm mềm các scale cực đoan và giữ tiny supervision ở mức hiệu quả hơn.
